...
const Person = () => {
  ...
  const addUser = () => {
    ...
    if (!userList.some(isSame)) {
      setuserList((prevList) => [
        ...prevList,
        {
          firstName: `${randomUser[0].name.first} ${randomUser[0].name.last}`,
          email: randomUser[0].email,
          phone: randomUser[0].phone,
          age: randomUser[0].dob.age,
        },
      ]);
    }
  };
    ...
    <>
      <div className="person">
        ...
        <div className="person-info">
          <p className="my-info">{info.title || "My name is"}</p>
          <h3 className="info">
            {!info.title
              ? `${randomUser[0]?.name.first} ${randomUser[0]?.name.last}`
              : info.detail}
